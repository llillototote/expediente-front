...
import {
  PositionRequest,
  PositionResponse,
} from 'src/services/api/bussiness/position/position.types';
import {
  createPosition,
  getByIdPosition,
  updatePosition,
...
async function onSubmit() {
  if (mode.value == 'add') {
    ...
    const resp: ResponseExternal<PositionResponse> = await createPosition(
      payload
    );
    if (resp.status == 201) {
      Notify.create({
        message: 'Correcto, puesto de trabajo creado satisfactoriamente!',
        textColor: 'white',
        color: 'green',
        position: 'top-right',
      });
      router.push({ name: NAMESROUTES.APP_POSITION_LIST });
    ...
    const resp = await updatePosition(id.value, payload);
    if (resp.status == 201) {
      Notify.create({
        message: 'Correcto, puesto de trabajo actualizado satisfactoriamente!',
        textColor: 'white',
        color: 'green',
        position: 'top-right',
      });
...
onMounted(async () => {
  ...
  if (mode.value == 'add') {
    title.value = 'Adicionar Puesto de Trabajo';
  } else {
    title.value = 'Modificar Puesto de Trabajo';
    id.value = route.query?.payload
      ? Number(route.query?.payload.toString())
      : null;
    if (id.value != null) {
      const recuperate = await getByIdPosition(id.value);
      if (recuperate.status == 200) {
        ...
        router.push({ name: NAMESROUTES.LOGIN });
      } else alert('puesto de trabajo no recuperado');
    }
  }
